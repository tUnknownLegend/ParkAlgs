...
public:
    ...
    void TraverseDfs(void (*visit)(int same));
...
private:
    ...
    void traverse(Node *node, void (*visit)(int same));
...
template<class T, class Compare>
void BinaryTree<T, Compare>::TraverseDfs(void (*visit)(int same)) {
    traverse(root, visit);
}

template<class T, class Compare>
void BinaryTree<T, Compare>::traverse(Node *node, void (*visit)(int same)) {
    if (node == nullptr) {
        visit(false);
        return;
    }
...
int main() {
    ...
    tree.TraverseDfs([](int same) { std::cout << same; });
    return 0;
}